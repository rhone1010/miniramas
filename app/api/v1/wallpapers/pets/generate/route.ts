// app/api/v1/wallpapers/pets/generate/route.ts
//
// PETS WALLPAPER - generate route.
//
...
//
// The engine has existed since the pets wallpaper room was built -
// wallpapers-pets.ts derives all 34 phone-composed bodies from the pets
// catalogue and registers them - but no route was ever put in front of it.
// The page declared this path and got a 404. This is the missing front
// door and nothing more: every decision about prompts, aspect, outpainting
// and retries already lives in generateWallpaper() and is not repeated
// here.
//
...
//
// EFFECT IDS ARE VALIDATED AGAINST THE PETS SET SPECIFICALLY, not the
// whole wallpaper registry. The engine would happily render a portraits
// or halloween effect if asked - getWallpaperEffect spans all rooms - and
// a pets page quietly rendering a halloween body because two ids collided
// is the kind of wrongness nobody reports, they just leave.

import { NextRequest, NextResponse } from 'next/server'
import { generateWallpaper } from '@/lib/v1/wallpapers/wallpapers-generator'
import { PETS_WALLPAPERS, PETS_WALLPAPER_IDS } from '@/lib/v1/wallpapers/wallpapers-pets'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!result.ok || !result.image_b64) {
      console.error(
        `[wallpapers/pets] failed effect=${effectId} ` +
    ...
    console.error('[wallpapers/pets] threw:', (e as Error).message)
