// app/api/v1/groups/raw-pipeline/route.ts
//
// Raw-prompt pipeline: NB2 only.
// No Pass 2 — gpt-image-1 smoothed NB2's textural character.
// No outpaint — Stability outpaint adds a white margin/matte that doesn't
//   match the close-up framing NB2 produces natively.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[raw-pipeline] start aspect=${aspect} ` +
      `prompt_chars=${prompt.length} additional=${(additional_images_b64?.length || 0)}`,
    )
    console.log(`[raw-pipeline] prompt: ${prompt.slice(0, 400)}${prompt.length > 400 ? '…' : ''}`)
    ...
    console.log(`[raw-pipeline] done in ${durationMs}ms`)
    ...
    return NextResponse.json({
      ...
      stages_run:    {
        pass1_nb2: true,
        pass2_gpt: false,   // removed — smoothed NB2 textural character
        outpaint:  false,   // removed — added unwanted white margin/matte
        faceswap:  false,   // never runs in raw mode
      },
    })
    ...
    const durationMs = Date.now() - t0
    console.error(`[raw-pipeline] failed in ${durationMs}ms: ${msg}`)
    return NextResponse.json(
...
// ─────────────────────────────────────────────────────────────
// callNB2 — inlined Replicate call (matches groups-generator.ts pattern)
// ─────────────────────────────────────────────────────────────
