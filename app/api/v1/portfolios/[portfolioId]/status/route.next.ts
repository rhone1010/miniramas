  ...
  const mappedItems = await Promise.all((items ?? []).map(async (i) => {
    ...
    if (i.preview_id && i.status === 'done') {
      ...
      if (unlocked) {
        ...
        // Gallery and detail share this authenticated, Foyer-marked image.
        // Existing stored tiled derivatives are left untouched.
        previewUrl = `/api/v1/portfolios/${encodeURIComponent(portfolio.id)}/art/${encodeURIComponent(i.preview_id)}`
      }
    }
