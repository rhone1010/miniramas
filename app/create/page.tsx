...
function LoadingScreen() {
  ...
  const steps = [
    { icon: '📷', label: 'Analysing photo' },
    { icon: '🏗️', label: 'Building scene' },
    { icon: '🎨', label: 'Applying finish' },
    { icon: '✨', label: 'Final render' },
  ]
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', width: '220px', height: '210px', marginBottom: '2rem' }}>
        ...
        {pathVisible && (
          <div style={{
            ...
            background: '#b0a080', borderRadius: '1px',
      ...
      <div style={{
        ...
        <div style={{
          height: '100%',
          background: 'linear-gradient(90deg, #7B5EA7, #00C9C8)',
          borderRadius: '10px',
          animation: 'progressSlide 30s ease forwards',
        }} />
      </div>
          ...
          <div key={i} style={{
            ...
            <div style={{
              ...
              background: activeStep === i
                ? 'linear-gradient(135deg, #7B5EA7, #00C9C8)'
      ...
      <div style={{ display: 'flex', gap: '6px', marginTop: '1.75rem' }}>
        {[
          { color: '#00C9C8', delay: '0s' },
          { color: '#7B5EA7', delay: '0.2s' },
          { color: '#FFD60A', delay: '0.4s' },
        ].map((d, i) => (
          <div key={i} style={{
  ...
  return (
    <main style={{
          ...
          {[1, 2, 3, 4].map((n) => (
            <div key={n} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{
                ...
                background: step === n
                  ? 'linear-gradient(135deg, #7B5EA7, #00C9C8)'
                  : step > n ? '#00C9C8' : 'rgba(255,255,255,0.08)',
              ...
              {n < 4 && (
                <div style={{
                  ...
                  background: step > n ? '#00C9C8' : 'rgba(255,255,255,0.08)',
                  borderRadius: '2px'
                }} />
              )}
            </div>
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        {step === 1 && (
          <div>
            <div style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#7B5EA7', marginBottom: '0.35rem' }}>
              Step 1 of 4
            </div>
            <h2 style={{ fontFamily: 'Nunito, sans-serif', fontSize: '1.9rem', fontWeight: 900, marginBottom: '0.4rem' }}>
              Drop your photo in 📸
            </h2>
            ...
            <label style={{
              display: 'block',
              border: '2px dashed rgba(0,201,200,0.3)',
              borderRadius: '16px',
              ...
              <div style={{ fontFamily: 'Nunito, sans-serif', fontSize: '1.1rem', fontWeight: 800, marginBottom: '0.3rem' }}>
                {previewUrl ? 'Change photo' : 'Drop your photo here'}
              </div>
            ...
            <button
              style={{ ...btnPrimary, opacity: !uploadedFile ? 0.4 : 1, cursor: !uploadedFile ? 'not-allowed' : 'pointer' }}
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.5)', marginBottom: '1.5rem', lineHeight: 1.65 }}>
              HoneScale has four specialised rendering modes. Pick the one that fits your photo best.
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '1.25rem' }}>
              {styles.map((s) => (
                <div
                  ...
                  style={{
                    border: style === s.id ? '2px solid #00C9C8' : '1px solid rgba(255,255,255,0.08)',
                    borderRadius: '16px', padding: '1rem', cursor: 'pointer',
                    background: style === s.id ? 'rgba(0,201,200,0.06)' : 'rgba(255,255,255,0.02)',
                  }}
                >
              ...
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' as const }}>
                {['realistic', 'cozy', 'cinematic', 'collectible'].map((m) => (
                  <button key={m} onClick={() => setMood(m)} style={{
                    fontSize: '0.72rem', fontWeight: 600, padding: '0.35rem 0.9rem',
                    borderRadius: '20px', border: mood === m ? 'none' : '1px solid rgba(255,255,255,0.1)',
                    background: mood === m ? 'linear-gradient(135deg, #7B5EA7, #00C9C8)' : 'transparent',
                    color: 'white', cursor: 'pointer', textTransform: 'capitalize' as const
                  }}>{m}</button>
                ))}
              </div>
            </div>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                {['clean', 'handcrafted', 'ultra'].map((d) => (
                  <button key={d} onClick={() => setDetail(d)} style={{
                    fontSize: '0.72rem', fontWeight: 600, padding: '0.35rem 0.9rem',
                    borderRadius: '20px', border: detail === d ? 'none' : '1px solid rgba(255,255,255,0.1)',
                    background: detail === d ? 'linear-gradient(135deg, #7B5EA7, #00C9C8)' : 'transparent',
                    color: 'white', cursor: 'pointer', textTransform: 'capitalize' as const
                  }}>{d}</button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                {['full', 'tight', 'wide'].map((c) => (
                  <button key={c} onClick={() => setComposition(c)} style={{
                    fontSize: '0.72rem', fontWeight: 600, padding: '0.35rem 0.9rem',
                    borderRadius: '20px', border: composition === c ? 'none' : '1px solid rgba(255,255,255,0.1)',
                    background: composition === c ? 'linear-gradient(135deg, #7B5EA7, #00C9C8)' : 'transparent',
                    color: 'white', cursor: 'pointer', textTransform: 'capitalize' as const
                  }}>{c}</button>
                ))}
              </div>
        ...
        {step === 4 && (
          <div>
            ...
            <h2 style={{ fontFamily: 'Nunito, sans-serif', fontSize: '1.6rem', fontWeight: 900, marginBottom: '0.5rem' }}>
              Your tiny world is ready! 🎉
            </h2>
            <p style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.5)', marginBottom: '1.25rem' }}>
              This is a watermarked preview. Unlock the full HD version below.
            </p>

            {error && (
              <div style={{ background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.3)', borderRadius: '12px', padding: '1rem', marginBottom: '1rem', fontSize: '0.82rem', color: '#FF9999' }}>
                {error}
              </div>
            )}
            ...
            {generatedImageUrl && (
              <div style={{ position: 'relative', marginBottom: '1.25rem' }}>
                <img
                  ...
                  style={{ width: '100%', borderRadius: '16px', border: '2px solid rgba(0,201,200,0.2)' }}
                />
                <div style={{
                  ...
                  <div style={{
                    fontSize: '1rem', fontWeight: 900, letterSpacing: '0.3em',
            ...
            <div style={{ background: '#141B35', borderRadius: '16px', padding: '1.25rem', marginBottom: '0.85rem', border: '1px solid rgba(255,255,255,0.06)' }}>
              <h3 style={{ fontFamily: 'Nunito, sans-serif', fontSize: '1.1rem', fontWeight: 900, marginBottom: '0.2rem' }}>
                Unlock your miniature 🔓
              </h3>
              <p style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.3)', marginBottom: '1rem' }}>
                Remove watermark · Full HD · Instant download
              </p>
              ...
              {[
                ...
                <div key={opt.name} style={{
                  ...
                  padding: '0.7rem 0.9rem', borderRadius: '10px', marginBottom: '6px',
                  ...
                  <div>
                    <div style={{ fontSize: '0.8rem', fontWeight: 700 }}>
                      ...
                      {opt.badge && (
                        <span style={{
                          fontSize: '0.55rem', fontWeight: 800, padding: '0.15rem 0.45rem',
                          borderRadius: '20px', marginLeft: '6px', verticalAlign: 'middle',
                          background: opt.badge === 'Popular' ? '#FFD60A' : '#FF6B6B',
                          color: opt.badge === 'Popular' ? '#0B1128' : 'white'
                        }}>{opt.badge}</span>
                      )}
                    </div>
                    <div style={{ fontSize: '0.64rem', color: 'rgba(255,255,255,0.3)', marginTop: '1px' }}>{opt.desc}</div>
                  </div>
                  <div style={{ fontFamily: 'Nunito, sans-serif', fontSize: '1.2rem', fontWeight: 900, color: '#00C9C8' }}>
                    {opt.price}
                  </div>
                </div>
              ))}
            ...
            <button style={btnGhost} onClick={() => { setStep(1); setUploadedFile(null); setPreviewUrl(null); setGeneratedImageUrl(null) }}>
