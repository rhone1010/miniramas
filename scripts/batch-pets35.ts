// scripts/batch-pets35.ts
//
// THE PETS THIRTY-FIVE. Thirty-four renders at 1:1, to H:.
//
...
//
//   npx tsx --env-file=.env.local scripts/batch-pets35.ts --dry
//   npx tsx --env-file=.env.local scripts/batch-pets35.ts
//   npx tsx --env-file=.env.local scripts/batch-pets35.ts --only clown
//   npx tsx --env-file=.env.local scripts/batch-pets35.ts --only bronze,iron,ebony
//
...
import { PETS_35, PETS_35_IDS } from '../lib/v1/pets/pets-catalog-35'
...
const OUT_ROOT = 'H:\\minramas\\public\\previews\\pets\\35'
...
async function main() {
  ...
  const manifestPath = join(OUT_ROOT, `manifest-pets35-${Date.now()}.json`)
