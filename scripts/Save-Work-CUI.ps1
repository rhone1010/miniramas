...
if ($branch -ne 'feature/store-commerce') {
  Write-Host ("REFUSED: on branch '" + $branch + "', expected feature/store-commerce.") -ForegroundColor Red
  Write-Host "Run: git switch feature/store-commerce   then re-run." -ForegroundColor Yellow
  exit 1
}
