...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ── Multi-subject (2–3 person) — ADDITIVE, read loosely ──────
    // Not yet a first-class field on PortraitsGenerateRequest (shared.ts left
    // untouched to avoid clobbering). Promote to the interface when ready.
    ;(generateRequest as any).subject_mode =
      body.subject_mode === 'multi' ? 'multi' : 'solo'
    ;(generateRequest as any).subject_count =
      Number(body.subject_count) || undefined
