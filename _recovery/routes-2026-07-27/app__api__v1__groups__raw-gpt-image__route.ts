// app/api/v1/groups/raw-gpt-image/route.ts
//
...
//
// This is a diagnostic tool — use when you want to know what gpt-image-1
// produces with a minimal prompt, without the full pipeline interfering.
// Logs the prompt + every request setting so you can compare against the
// full-pipeline renders.

import { NextRequest, NextResponse } from 'next/server'
import fs        from 'node:fs/promises'
import path      from 'node:path'
import { callGptImage1 } from '@/lib/v1/groups/groups-gpt-image'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Path is validated to stay under that directory (no path traversal).
    ...
    if (style_reference_path) {
      ...
      try {
        ...
        console.log(`[raw-gpt-image] loaded style ref: ${cleaned} (${buf.length} bytes)`)
      } catch (e: any) {
        console.warn(`[raw-gpt-image] style ref load failed (${cleaned}): ${e?.message}`)
        // Continue without the ref — degrade gracefully.
      }
    }

    console.log(
      `[raw-gpt-image] start aspect=${aspect_ratio || '1:1'} ` +
      `quality=${quality || 'medium'} ` +
      `style_refs=${styleRefB64s.length} ` +
      `prompt_chars=${prompt.length}`,
    )
    console.log(`[raw-gpt-image] prompt: ${prompt.slice(0, 400)}${prompt.length > 400 ? '…' : ''}`)
    ...
    console.log(`[raw-gpt-image] done in ${durationMs}ms`)
    ...
    const durationMs = Date.now() - t0
    console.error(`[raw-gpt-image] failed in ${durationMs}ms: ${msg}`)
    return NextResponse.json(
