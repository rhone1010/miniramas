...
//
//   width    LOCKUP_WIDTH of the image width -- 75% of the approved 62%
//            (Rich, 2026-09-15: "shrink by 25%"), centred left to right
...
//
// UNDER the lockup, white lines on the geometry of the foyer's earlier
// PREVIEW treatment (the glass's .wm overlay: a 3px line on the 442px
// desktop card), without the word PREVIEW. Lighter since 2026-09-15 (Rich:
// the crosshatch was too heavy -- one direction, dashed, 50% further apart):
//
//   direction  LINES_ANGLES: one set, running down from right to left
//              (top-right to bottom-left)
//   period     LINES_PERIOD of the image width (33/442 = 7.47%: 63px at 848,
//              the earlier 22/442 x 1.5)
//   thickness  LINES_WIDTH of the image width  (3/442 = 0.68%: 6px at 848)
//   dashes     LINES_DASH on, LINES_GAP off, along each line (6 and 3 line
//              widths: 35px and 17px at 848)
...
export const LOCKUP_WIDTH    = 0.62 * 0.75   // 75% of the approved 62% (0.465)
...
export const LINES_PERIOD    = 33 / 442 // of the image width -- the glass .wm's 22px on its 442px card, 50% further apart
export const LINES_WIDTH     = 3 / 442  // of the image width -- its 3px line
export const LINES_DASH      = 6 * LINES_WIDTH   // of the image width, along the line
export const LINES_GAP       = 3 * LINES_WIDTH
export const LINES_OPACITY   = 0.18
export const LINES_ANGLES    = [45] as const  // SVG rotate of vertical stripes: top-right to bottom-left
...
/* The dashed diagonal lines, one full-size layer. Each set is the stripes
   of vertical bars rotated by its angle -- u = x cos a + y sin a in
   [k period, k period + width] -- drawn as stroked, dashed lines along their
   centres (a pattern fill draws the same stripes ~30x slower). */
export function foyerLinesGeometry(imageWidth: number): { period: number; width: number; dash: number; gap: number } {
  return { period: imageWidth * LINES_PERIOD, width: imageWidth * LINES_WIDTH, dash: imageWidth * LINES_DASH, gap: imageWidth * LINES_GAP }
}
function linesSvg(imageWidth: number, imageHeight: number): Buffer {
  const g = foyerLinesGeometry(imageWidth)
  const reach = imageWidth + imageHeight, span = Math.ceil(reach / g.period) + 1
  const f = (n: number) => n.toFixed(2)
  const sets = LINES_ANGLES.map(a => {
    const nx = Math.cos(a * Math.PI / 180), ny = Math.sin(a * Math.PI / 180)
    let d = ''
    for (let k = -span; k <= span; k++) {
      const c = k * g.period + g.width / 2, cx = c * nx, cy = c * ny
      d += `M${f(cx + reach * ny)} ${f(cy - reach * nx)}L${f(cx - reach * ny)} ${f(cy + reach * nx)}`
    }
    return `<path d="${d}"/>`
  })
  return Buffer.from(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${imageWidth}" height="${imageHeight}">` +
      `<g opacity="${LINES_OPACITY}" stroke="#ffffff" stroke-width="${g.width}" stroke-dasharray="${f(g.dash)} ${f(g.gap)}" fill="none">${sets.join('')}</g>` +
    `</svg>`,
  )
}
