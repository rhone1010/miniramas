...
// white, without the word PREVIEW (Rich, 2026-09-14) -- crossed: the same
// set again in the opposite diagonal, a white X pattern (Rich, 2026-09-14,
// "cross-angled means BOTH diagonal directions"):
//
//   directions LINES_ANGLES: top-left to bottom-right, as the CSS drew it,
...
//   ink        white at LINES_OPACITY (.18), both sets as one layer, so a
//              crossing is no brighter than a line
//
...
export const LINES_ANGLES    = [-45, 45] as const  // SVG rotate of vertical stripes: top-left to bottom-right (as CSS 45deg draws), and its mirror
...
/* The crossed diagonal lines, one full-size layer. Each set is the stripes
   of vertical bars rotated by its angle -- u = x cos a + y sin a in
   [k period, k period + width] -- drawn as stroked lines along their
...
function linesSvg(imageWidth: number, imageHeight: number): Buffer {
  ...
  return Buffer.from(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${imageWidth}" height="${imageHeight}">` +
      `<g opacity="${LINES_OPACITY}" stroke="#ffffff" stroke-width="${g.width}" fill="none">${sets.join('')}</g>` +
    `</svg>`,
  )
}
