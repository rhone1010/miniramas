...
//
// Nothing tiles. Everything is a fraction of the image, so it lands in the
// same place at whatever size NB2 returns; the image's own dimensions are
// kept. FAIL CLOSED, as bakeWatermark does: any failure throws, and the
// caller treats it as a failed reveal -- never a clean image.
//
...
export async function bakeFoyerWatermark(imageB64: string): Promise<string> {
  ...
  const [shadow, mark] = await Promise.all([
