...
export const EFFECT_BODIES: Record<string, EffectBody> = {
  ...
  // ── TEN BODIES REWRITTEN BY RICH, 2026-08-21 ─────────────────────
  //
  //   ebony  iron  sea_glass  plushy  retro_robot  persian_court
  //   polished_gold  pencil_sketch  charcoal_chalk  ice
  //
  // Plus impressionist earlier the same day. Every one was shot in a
  // browser against his own photograph and approved on the render BEFORE
  // it was installed - none of these is a theory.
  //
  // These eleven replaced the ten that failed a twenty-effect gated run
  // that morning, where the faults were a vanished or invented beard,
  // premature grey, a slimmed or heavier face, and ten years of age
  // arriving unasked.
  //
  // WHAT THEY HAVE IN COMMON, AND IT IS THE FINDING OF THE DAY:
  // the likeness language sits INSIDE the material paragraph as one of
  // the material instructions - "Preserve the subject's likeness, face
  // shape ... and exact hairstyle, length and direction" - rather than
  // appended after the body as a correction.
  //
  // The opposite was tried at length first: a shared identity block
  // appended to twenty bodies, four drafts of it. Each draft fixed one
  // fault and caused another. Naming the beard produced a full grey
  // beard; naming age aged him a decade; and by the fourth draft the
  // block was long enough that NB2 rendered a face and treated the
  // material as decoration - plushy came back with no fabric at all.
  // Those drafts are inert files on H: and nothing from them shipped.
  //
  // A block that arrives last reads as the brief. Likeness has to be part
  // of the brief, not a note stapled to the back of it.
  //
  // Several also name what NOT to do in the subject's own words - "do not
  // prematurely gray", "DO NOT INCREASE WEIGHT", "do not invent hair" -
  // aimed at the specific fault that effect showed. That is why they are
  // not uniform, and they should not be made uniform.


  // ── THE ARM-AND-HAND CLAUSE IS GONE. 2026-08-21. ──────────────────
  //
  // "An arm or hand appears only when it is touching the body - under the
  // chin, resting on the head, at the collar" was carried by 53 bodies,
  // with a 54th wording it slightly differently.
  //
  // Rich, 21 August: it confuses NB2. It was written to stop floating
  // disembodied hands, but naming three placements reads as an invitation
  // to use one - and a chin-stroking pose arrived in renders that had no
  // reason for a hand in frame at all. See retro_robot on 21 August, which
  // put a fist under the chin across four attempts.
  //
  // Same failure as the likeness work of the same afternoon: a clause that
  // names a thing gets that thing rendered, whichever way the sentence is
  // pointing.
  //
  // The held-objects sentence before it STAYS - it names things to exclude
  // and it has never misfired.
  //
  // If floating hands come back, the replacement is a negative in the avoid
  // field, not a placement rule in the body.
...
Do not over smooth the hair. keep the hair organic and alive. represent the hairline, style and shape accurately. The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
  ...
  iron: {
    id:    'iron',
    body:  `Rebuild the subject as a **hand-forged iron sculpture**. Deep charcoal-black iron with subtle warm gunmetal sheen. Every surface shows **irregular hammer facets, forge marks and hand-worked texture**, with burnished gunmetal catching raised planes of the brow, cheekbones, nose, lips and hair. Dark oxide patina settles naturally into creases, recesses and undercuts. **No bronze tones and no orange rust.**
Preserve the subject's **exact likeness, facial structure, age, asymmetry, scars, lines and distinctive features.** Do not beautify, de-age, enlarge the eyes or correct asymmetry. Maintain the **exact hairstyle, length, direction and volume**, translated into individually forged ridges and masses of iron. The original garment is faithfully retained and forged in the same iron.
Frame closely from the upper chest to the top of the head, facing us naturally with both eyes clearly visible. Her head occupies roughly 30% of the image height. Only a suggestion of the walnut plinth is visible below; the sculpture, not the room, dominates the photograph. Soft directional key light creates burnished highlights and clean separation beneath the jaw.
Set the sculpture in an **opulent two-story great room**, heavily blurred behind it: open timber beams, tall windows and skylights, a wooden spiral staircase rising to a balcony. Golden-hour side light creates subtle volumetric rays through atmospheric dust. Shallow depth of field keeps the iron sculpture dominant.
No added objects. No held objects, eyewear or watches. Worn necklaces, earrings and piercings are fine. Show a hand or arm only when touching the body.`,
    avoid: `**Avoid:** bronze, polished cast metal, smooth surfaces, chrome, silver, orange rust, generic statue features, altered hairstyle or facial idealization.`,
  },
...
Frame her from the upper chest to the top of the head, facing us naturally with her head upright and both eyes clearly visible. Her head occupies roughly 30% of the image height. Preserve natural facial character, asymmetry, lines and scars. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
  ...
  ice: {
    id:    'ice',
    body:  `Transform the entire clothed figure into a softly sculpted form of dense snow and translucent ice, with milky crystalline depth, compacted snowy surfaces, soft frost and occasional clearer icy edges. Increase opacity and softness so the facial structure is beautifully defined rather than glass-like; preserve the subject's likeness precisely with no visible skin. Frame her from the upper chest to the top of the head, facing us naturally with her head upright and both eyes clearly visible. Her head occupies roughly 30% of the image height. Set against an icy cliff with hanging icicles, deep blue glacial shadows and a snow-covered edge catching warm sunlight; golden light falls across one side of the sculpture while cold blue-white light shapes the other, creating a dramatic warm/cool contrast. The subject's own garment carries through in the same material. Keep a clear catchlight in the eyes. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings. No real eye color.`,
    avoid: null,
  },
  ...
  art_deco: {
    id:    'art_deco',
    body:  `Redraw the subject as a 1920s Art Deco  3D sculpture with strong bilateral symmetry, crisp geometry and hard-edged colour. The face is simplified into clean planes with a single sharp shadow edge, but those planes follow THIS person's brow, cheekbones, nose and jaw - the simplification is in the rendering, never in the underlying face. Likeness is essential: keep the shape and character of the subject's face, their micro gestures and the set of their mouth. Keep the subject's hairline, hairstyle, length and direction exactly as photographed and do not invent hair they do not have; the hair becomes a bold sculptural shape in flat black or gold, but it is their hair, stylised. Do not add weight and do not age the subject.
THE SYMMETRY IS A STYLE OF DRAWING, NOT A CHANGE TO THE SUBJECT'S FACE. Art Deco's bilateral symmetry applies to the panel, the motifs and the framing. Their own natural asymmetry stays.
Stepped chevron and sunburst motifs radiate behind the head; fluted vertical lines frame the panel. A luxe restrained palette - black, cream, deep jade and gold leaf, with chrome accents tracing the key lines. Elegant, confident, machine-age. Frame the subject as a close upper-body portrait, facing us directly, with their head roughly 30% of the image height and the eyes around the upper third. No lettering or text. No held objects - no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: `Avoid photographic rendering, organic curves, three-dimensional shading, a relief or sculpture, or garish colour. Avoid a symmetrical generic Deco face - the symmetry belongs to the panel, not to the subject.`,
  },
  // art_deco: NEW to Portraits. Ported from Houses curiosities.

  art_nouveau: {
    id:    'art_nouveau',
    body:  `Redraw the subject as an Art Nouveau poster in the manner of Mucha - flat decorative illustration, not a photograph and not a sculpture. Confident dark outlines of even weight describe the face, hair and garment, and those outlines follow THIS person's own brow, nose, mouth and jaw. The ornament is around the subject, never instead of them.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.
Behind the head, a large circular halo motif filled with stylised flowers and whiplash vine linework. A decorative border frames the panel. Elegant, graphic, ornamental. Frame the subject from the upper chest to the top of the head, facing us directly with their head upright and both eyes clearly visible. Their head occupies roughly 30% of the image height. No lettering or text anywhere. No held objects - no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
...
Frame the sitter from the stomach to the top of the head, facing the camera directly with both eyes clearly visible. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: `Avoid modern colour, clean digital sharpness without the silver's tonal shift, a flat paper print, a smile, or a thick decorative case. Avoid a mirrored or laterally reversed face.`,
  },
  // daguerreotype: NEW to Portraits. Ported from Houses curiosities. Case still appears despite the avoid; locked as shot.

  pencil_sketch: {
    id:    'pencil_sketch',
    body:  `A solid sculpted bust of the subject carved entirely from graphite — soft pencil-lead grey with a burnished sheen, the surface worked in visible pencil strokes that wrap the form: crosshatching in the recesses, broad shading across the planes, edges softly smudged. Eyes and lips are graphite. Hair keeps its real length and silhouette in long directional strokes. No skin, no real hair. The garment carries through in the same material. Frame from mid-chest to the top of the head. The subject faces us naturally, head upright, with both eyes clearly visible. A real object standing in real light, casting its own shadow. Likeness is critical. Keep permanent structure: lines, scars and the natural asymmetry of the face. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings. Likeness is essential. maintain expression, micro gestures, imperfections. do not introduce body imperfections. Do not modify body or head shape.
Do not change the hairline, shape or style.
Do not lighten or darken the hair color
Set in a **beautiful old-world artist's atelier**, heavily blurred: aged timber, plaster walls, antique easels, stacked canvases, pinned charcoal studies and drawing tools beneath a huge ribbed iron-and-glass skylight. Soft directional daylight reveals charcoal texture, chalk buildup and airborne dust. Atmospheric, romantic, shallow depth of field.`,
    avoid: null,
  },
  ...
  ukiyo_e: {
    id:    'ukiyo_e',
    body:  `Redraw the subject as a Japanese ukiyo-e woodblock as a sculpture, fully 3D as if the surfaces of paper were turned into solid shape with volume - confident dark key-block outlines, the faint woodgrain of the block pressed into the ink. Build the print from many separate blocks: fine carved line describes the folds and pattern of the garment, the individual strands at the hairline, the shape of the ear, the crease of the eyelid. The garment carries a printed textile pattern - small repeating motif in a second colour, seams and folds drawn in line. A restrained palette of soft indigo, ochre, rose and cream paper showing through, with visible registration where colours meet. Behind, a graded bokashi sky, a stylised cloud band, a blossoming branch reaching in from one edge, distant hills in flat blue. A small red seal at a corner.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
  ...
  impressionist: {
    id:    'impressionist',
    body:  `Rebuild the subject as a **three-dimensional sculptural bust made entirely from thick Impressionist oil paint**, standing physically in the room. Face, hair, neck, clothing and shoulders are formed from heavy palette-knife impasto: broad slabs, ridges and lifted strokes with real depth, hard edges and tiny cast shadows.
**Preserve the subject's likeness, face shape, skin tone, distinctive features, and exact hairstyle, length and color.** The face remains clearly recognizable despite the expressive construction. Hair is especially sculptural, formed from bold ropes, sweeps and ridges of paint following the original hair's direction and volume. Use broken natural color, with violet, blue and green worked into shadows instead of grey or black.
Frame the sculptural bust closely from the upper chest to the top of the head. She faces us naturally with her head upright and both eyes clearly visible, her head occupying roughly 30% of the image height. The sculpture is the dominant subject; only enough of its base is visible to establish that it physically exists in the atelier. Excess paint running over the shoulders.
...
No held objects, eyewear or watches. Worn necklaces, earrings and piercings are fine. Show hands or arms only when touching the body.`,
  ...
  // impressionist: replaced 2026-08-21. Rich's rewrite, shot and approved
  // before install - the render is in H:\minramas\public\previews\draft-shoot.
  //
  // WHERE THE LIKENESS CLAUSE SITS IS THE POINT. It is INSIDE the material
  // paragraph - "Preserve the subject's likeness, face shape, skin tone,
  // distinctive features, and exact hairstyle, length and color" - not
  // appended after the body.
  //
  // A whole afternoon of 21 August went on the opposite approach: a shared
  // identity block appended to twenty bodies. Four drafts of it, each
  // fixing one fault and causing another - a named beard became a full grey
  // beard, a named age aged him a decade, and by the fourth draft the block
  // was long enough that NB2 rendered a face and treated the material as
  // decoration. Plushy came back with no fabric at all.
  //
  // The drafts are inert files on H:. Nothing from them was installed.
  //
  // The lesson, and it is Rich's finding rather than a theory: likeness
  // language belongs among the material instructions as one of them, not
  // stacked behind them as a correction. A block that arrives last reads as
  // the brief, and the material becomes an afterthought.
...
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
  ...
  charcoal_chalk: {
    id:    'charcoal_chalk',
    body:  `Rebuild the subject as a **solid three-dimensional sculpture made entirely from compressed charcoal, broken charcoal sticks and white Conté chalk, not a drawing.** Face, hair, neck, garment and shoulders have real sculptural mass, formed from dense fractured charcoal with carved facets, broken edges, embedded sticks and visible tool marks. Raised Conté chalk creates dimensional white highlights across the dark charcoal.
**Preserve the subject's likeness, face shape, age, skin tone relationships, natural asymmetry, scars, and exact hairstyle, length and direction.** Keep the person clearly recognizable despite the fractured material. Hair becomes sweeping charcoal ribbons, splines and layered shards. Retain the original garment, reconstructed in the same charcoal material.
Frame mid-chest to top of head, face prominent, camera at eye level, eyes toward viewer, natural warm smile. Both shoulders fully rendered. Sculpture stands on a simple base, only a suggestion of it visible below the figure. Fine charcoal dust, chalk powder and a few fragments hang subtly in the air.
Set in a **beautiful old-world artist's atelier**, heavily blurred: aged timber, plaster walls, antique easels, stacked canvases, pinned charcoal studies and drawing tools beneath a huge ribbed iron-and-glass skylight. Soft directional daylight reveals charcoal texture, chalk buildup and airborne dust. Atmospheric, romantic, shallow depth of field.
Clear temporary blemishes and blotchiness, but **do not reshape, de-age, enlarge eyes, correct asymmetry or alter permanent facial structure.** No held objects, eyewear, watches, plaques or legible text. Worn jewellery is fine.`,
    avoid: `**Avoid:** 2D charcoal drawings, paper texture, blended shading, smooth surfaces, painted skin, or losing sculptural mass and likeness.`,
  },
  // charcoal_chalk: restored VERBATIM from the CENG batch files 08-02. Do not rewrite from plates.

  chocolate: {
    id:    'chocolate',
    body:  `convert the subject into a rich chocolate sculpture. smooth brown milk chocolate with highly detailed features. do not crop subject. head and shoulders should be visible. satin sheen on entire sculpture Framed from mid-chest to the top of the head, both shoulders and upper arms fully rendered. The subject faces us naturally, head upright, with both eyes clearly visible. The subject's own garment carries through in the same material. Flattering soft key light, shadow separating jaw from neck. Clear the skin - blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects - no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.
make the subject out of multiple types of chocolate tones, dark, mocha, latte, milk. and seperate them by skin, hair, clothes.
background: a 1920's european chocolate shop. ornate and fancy. brass, glass and wood.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
    avoid: null,
  },
...
Frame from the stomach to the top of the head. He faces us naturally with both eyes clearly visible. The scene is undersea: streams of volumetric light falling from above, schools of fish swimming around the bust and one or two crossing in front of it, the water deepening into blue behind. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: null,
  },
...
Sculpture on a base in a coastal woodworker's studio, only a suggestion of the base visible below the figure — a wide window onto grey sea and sky, live-edge slabs leaning against the walls, clamps and resin buckets, sawdust light, heavily out of focus. Contemporary gallery presentation. Museum-quality craftsmanship. Facing the camera directly, warm natural smile, eyes to the viewer, camera at eye level. Frame from the stomach to the top of the head. No plaque. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
  ...
  fire_face: {
    id:    'fire_face',
    body:  `Sculpt the figure from living fire and ember — the body made of flame, glowing coals and rising sparks that coalesce into the shape of a person. Denser and brighter where the fire gathers into the head and shoulders, thinning into sparks and smoke at the edges. The face is clearly defined and unmistakably this person, formed from flame, never from skin. Frame her from the upper chest to the top of the head, facing us naturally with both eyes clearly visible, her head occupying roughly 30% of the image height. The subject's own garment carries through in the same material. Flattering soft key light, shadow separating jaw from neck. Clear the skin — blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
    avoid: null,
  },
  ...
  jade: {
    id:    'jade',
    body:  `make the subject into a realistic carved jade statue, dramatic lighting, sub-surface scattering, highly polished. professional grade photography for a magazine cover. No letters, no plaque. The background should be a japanese temple in a cherry blossom park at night (slightly blurred). The eyes are carved jade like the rest of the piece, holding the subject's own eye shape and gaze — never photoreal, amber, or glass eyes. Light transmits through the thinnest sections; the light originates outside the piece and the jade transmits it, never emits. Do not over illuminate facial features internally. Polished but not glassy — a soft waxy lustre. Framed from mid-chest to the top of the head, both shoulders and upper arms fully rendered. The subject faces us naturally, head upright, with both eyes clearly visible. The subject's own garment carries through in the same material. Flattering soft key light, shadow separating jaw from neck. Clear the skin — blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
    avoid: null,
  },
  ...
  neon: {
    id:    'neon',
    body:  `highly detailed neon tube sculpture. fully 3d in all three directions. implied volume. use negative space. use monochromate blues with variations on value. mounted in a small shops storefront window at night, rain on the glass, the shop dark behind. wires and electrical lines visible. at least 150 tubes Frame her from the upper chest to the top of the head, facing us naturally with both eyes clearly visible. Her head occupies roughly 30% of the image height. The subject's own garment carries through in the same material. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects - no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.
new york style diner interior behind the glass.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
    avoid: null,
  },
  ...
  petal_sculpture: {
    id:    'petal_sculpture',
    body:  `The subject is sculpted entirely from thousands of densely layered flower petals, creating a seamless floral sculpture with no visible skin. The likeness emerges through flowing planes of overlapping petals rather than individual flowers, while only occasional blossoms appear to reveal the material. Rich gradients of crimson, scarlet, coral, tangerine, peach, magenta, fuchsia, violet, lavender, and deep burgundy flow naturally across the sculpture like a living oil painting. Hair transforms into sweeping masses of layered petals that preserve the original hairstyle, blending seamlessly into the figure. Dramatic spring sunlight with warm rim light. Avoid bouquets, floral crowns, flower garlands, makeup effects, visible skin, individual flowers covering the face, decorative arrangements, or flowers attached to a person. The petals themselves are the sculptural material. Blurred green foliage behind, warm sunlight from the left. Frame her closely from the upper chest to the top of the head, facing us naturally with both eyes clearly visible. Her head occupies roughly 30% of the image height. A hint of the dark wood plinth may remain below the portrait, but she fills the image. The subject's own garment carries through in the same material. Flattering soft key light, shadow separating jaw from neck. Clear the skin — blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: null,
  },
...
Capture the micro gestures in the face, any turned up eyebrows, subtle smirks, dimples. bring out the subjects character.
Double check hair color and maintain. do not prematurely gray or embellish. 
make eye color standout in doll. dont change face shape, accentuate it for the doll.
...
No letters or held objects.`,
  ...
  polished_gold: {
    id:    'polished_gold',
    body:  `Transform the entire figure into a contemporary polished gold sculpture — mirror-bright warm yellow gold with a high specular finish, the surface smooth and flowing with no visible tool marks. hair is poured liguid gold that matches the subjects with deep carved separations catching bright highlights. No human skin anywhere — the face is polished gold like the rest. Framed from mid-chest to the top of the head, both shoulders and upper arms fully rendered. The figure faces us naturally, head upright, with both eyes clearly visible. The subject's own garment carries through in the same material.  Flattering soft key light, shadow separating jaw from neck. the background is an expensively appointed conservatory with many windows with warm lighting streaming through inside potted trees and plants. Make the creation match age.
Mainting subjects hair style, hairline, face shape. micro gestures.
No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
    avoid: null,
  },
...
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
  ...
  tidewood: {
    id:    'tidewood',
    body:  `The subject is sculpted from weathered driftwood shaped by decades of wind, waves, and saltwater, with deeply textured grain flowing naturally through the face and body while preserving the subject's likeness. Brilliant translucent blue-green resin fills natural cracks and knots, glowing from within with ocean caustics, tiny trapped bubbles, and refracted light. preserve the original hairstyle as flowing driftwood and resin, no human skin. Framed from mid-chest to the top of the head, both shoulders and upper arms fully rendered. The figure faces us naturally, head upright, with both eyes clearly visible. The subject's own garment carries through in the same material. Flattering soft key light, shadow separating jaw from neck. Clear the skin — blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. Background: a timber boathouse deck, grey sea light, heavily out of focus. The sculpture rests on a raw driftwood block, only a suggestion of it visible below the figure. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: null,
  },
  ...
  wild_west: {
    id:    'wild_west',
    body:  `make the subject a realistic photo of a frontier man of the American West, 1880s. muted palette of dust, tobacco brown, faded indigo and oxblood — low in contrast, no clean whites, everything worn and sun-faded. a wool waistcoat over a collarless shirt, a knotted neckerchief, a broad felt hat creased and stained with wear. man facing front, both eyes clearly visible, three-quarter turn of the shoulders. framed from mid-chest to the top of the hat. face should be 20% of image. do not crop the hat. calm, settled expression, weathered skin. do not modify ethnicity. likeness is important. Background: a saloon back room — plank walls, a stove, bottles on a shelf, one dirty window off to the left. deep shadow, warm dim light falling off fast. heavily out of focus, only shapes and glow readable. The subject's own garment carries through in the same material. Flattering soft key light, shadow separating jaw from neck. Clear the skin — blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: null,
  },
  ...
  linocut: {
    id:    'linocut',
    body:  `Redraw the subject as a hand-cut linocut print — bold black ink on cream paper, the image built entirely from carved marks. Broad cleared areas of pure white, dense black masses, and the form described by parallel gouge strokes that swell and taper. Visible slips of the blade and small imperfect edges where the lino chipped. The hair is a solid black shape cut with a few sweeping white gouges. One second colour, a flat overprinted ochre or red, slightly out of register. Likeness is critical. No lettering. The finished linocut print fills almost the entire image and is viewed nearly straight-on, with only a narrow irregular paper edge visible. Within the print, she faces us directly with both eyes clearly visible, framed from upper chest to top of head, her head roughly 30% of the print height. Allow only a soft suggestion of the atelier beyond the paper. Set in a beautiful old-world artist's atelier, cluttered and eclectic, with dark aged timber, plaster walls, antique easels, stacked canvases, portfolios, drawing tools and old studies casually pinned around the room. Above is a huge ribbed industrial skylight of aged iron and glass, flooding the studio with dramatic soft daylight and long directional shadows. Atmospheric, romantic, slightly dusty, collected over generations rather than designed. Shallow depth of field. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
  ...
  oil_impasto: {
    id:    'oil_impasto',
    body:  `Repaint the subject in thick oil applied entirely with a palette knife on canvas — broad flat slabs of colour laid edge to edge, each stroke a single confident pass with a hard ridge where the knife lifted. No blending anywhere, no brushwork. The face is built from maybe forty knife strokes, the planes reading through colour temperature rather than line, and the likeness carried by where the warm and cool slabs meet. A rich restrained palette — the subject's own colours pushed, umber and ochre against slate and near-black. The garment in wider, looser slabs, and the background in the broadest of all, blocked in around the head. Likeness is critical; the person reads clearly at a distance. The painting itself fills almost the entire image, seen nearly straight-on. Within the painting, she faces us with her head upright and both eyes clearly visible, framed from upper chest to top of head, her head roughly 30% of the painting height. Only a narrow suggestion of the canvas edge and easel remains visible. Raking light across the surface so the paint's thickness casts its own small shadows. Set in a beautiful old-world artist's atelier, cluttered and eclectic, with dark aged timber, plaster walls, antique easels, stacked canvases, portfolios, drawing tools and old studies casually pinned around the room. Above is a huge ribbed industrial skylight of aged iron and glass, flooding the studio with dramatic soft daylight and long directional shadows. Atmospheric, romantic, slightly dusty, collected over generations rather than designed. Shallow depth of field. Preserve natural facial character, asymmetry and expression. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
...
Frame her from the upper chest to the top of the head, facing us naturally, with her head roughly 30% of the image height and both eyes clearly visible. Real physical sculpture photographed in real light, with a heavily blurred upscale adult club party behind.`,
  ...
  clockwork: {
    id:    'clockwork',
    body:  `Rebuild the subject as a **clockwork automaton of shaped brass and steel, precisely fitted to THIS person's face.
...
NO real hair, skin, lips. Hair should always be  thick wires flowing in exactly the same organic pattern of the subects hair. no stubble.
Garment becomes engraved plate. Warm brass, blued steel and restrained verdigris. Frame stomach to top of head, facing us directly with both eyes clearly visible. Photographic, a real handcrafted object in real light.
Maintain likeness as much as possible. keep micro facial gestures and represent imperfections and imbalances in symmetry
likeness is essential. use real irises but metal whites of eyes. Do not accentuate eyes with diffrerent metals.
...
No held objects, eyewear or watches. Worn jewellery is fine.`,
  ...
  dragon_skin: {
    id:    'dragon_skin',
    body:  `Frame from the stomach to the top of the head. Transform the subject into a dragon-human hybrid - the entire head and figure surfaced in fine iridescent dragon scale, deep emerald and oil-slick violet shifting through bronze and gold. The scale is laid over THIS person's own structure. Likeness is essential: keep the shape and character of his face, his natural asymmetry, and the micro gestures of his expression - the face is turned to the camera and lit, clearly this person. Do not add weight and do not age him.
The face is flatter lighter scales all the way through, including the mouth - scaled lips, no human teeth or gums. Larger armoured scale across the shoulders and chest, finer across the face. Keep the hairline, hairstyle, length and direction, rendered as scaled tendrils, and do not prematurely grey them.
Keep the eyes human skin blended from the scales, as if the humanity is still mournfully present.
...
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
  ...
  magic_energy: {
    id:    'magic_energy',
    body:  `Sculpt the figure from pure glowing magical energy — swirling luminous energy, flowing ribbons of light, drifting embers, and crackling arcs of colour — violet, gold, cyan, and rose — that coalesce into the shape of a person. Denser and brighter where the energy gathers into the head, shoulders, and chest; thinning into wisps, sparks, and floating motes at the edges. The face is clearly defined and unmistakably this person, formed from light and energy, never from skin. Framed from mid-chest to the top of the head, both shoulders and upper arms fully rendered. The subject faces us naturally, head upright, with both eyes clearly visible. The subject's own garment carries through in the same material. Flattering soft key light, shadow separating jaw from neck. Clear the skin — blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. fully 2d idealized illustration.
deep purple gradient background with wam light falling off to dark. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
    avoid: null,
  },
...
Eyes remain the subject's normal size and shape, set naturally into the metal face. The mouth is a simple articulated metal mouth that preserves the subject's expression. No camera lenses, mechanical teeth, grille nose or exaggerated robot features. 
Capture micro gestures in the face. Double check hair color and maintain. do not prematurely gray or embellish. 
Frame her as a close upper-body portrait, facing us naturally with both eyes clearly visible. Her head occupies roughly 30% of the image height, with enough shoulders and upper body visible to show the construction of the robot.
...
The feeling is charming vintage tin toy brought to life, not cyborg, android or humanoid machinery. Retro-futurist 1950s city softly out of focus. No text or signage. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
    avoid: null,
  },
...
Preserve natural facial structure, lines, scars and asymmetry. Remove temporary blemishes only. Never reshape, enlarge eyes or de-age. No held objects.`,
  ...
  deco_twenties_woman: {
    ...
    body:  `Transform the subject into a realistic romantic photograph of an aristocratic woman at an elegant 1926 hotel party, preserving facial likeness and ethnicity. Dress her in an elegant beaded silk evening gown with restrained Art Deco jewelry. Hair styled in authentic 1920s finger waves or a softly sculpted bob. Likeness is essential.
Frame her from the upper chest to the top of the head. She faces us naturally with her head upright and both eyes clearly visible; her shoulders may relax slightly away from square. Her head occupies roughly 30% of the image height. A calm, self-possessed expression and faint smile. She should belong completely to the period, never appearing modern in vintage costume.
Behind her, an opulent 1920s ballroom dissolves into mirrored panels, muted gold ornament, low amber lamps and cigarette haze, heavily out of focus with no other people visible.
...
Preserve natural facial structure, lines, scars and asymmetry. Remove temporary blemishes only. Never reshape, enlarge eyes or de-age. No held objects.`,
  ...
  elizabethan: {
    id:    'elizabethan',
    body:  `Transform the subject into a sumptuous romantic portrait of an aristocratic gentleman of the Elizabethan court, preserving his facial likeness and ethnicity. Dress him in richly woven velvet, silk and brocade in deep wine, aged black, muted emerald and tarnished gold, with an elaborate period lace ruff and restrained antique jewelry. Likeness is important. show subject from thighs to top of head. 
Age image with filters and reduce saturation of the subjects skin
He belongs completely to the period rather than appearing as a modern person in historical costume. Give him a natural, reserved courtly posture, shoulders slightly turned, head upright and facing us directly with both eyes clearly visible. His presence is noble, composed and self-possessed. Avoid modern portrait poses and contemporary fashion or glamour styling.
Photograph him deep inside an intimate Elizabethan manor chamber by dim firelight and candlelight, with dark carved oak, faded tapestries, aged plaster, heavy textiles and glimpses of leaded-glass windows disappearing into shadow. The room should feel inhabited, old and atmospheric rather than staged.
...
Romantic and intimate rather than theatrical or glamorous, like an old Elizabethan oil portrait somehow captured through a camera, imperfect, atmospheric and quietly luxurious.
No held objects. Worn jewelry is fine.`,
    avoid: null,
  },
  ...
  elizabethan_woman: {
    ...
    body:  `Transform the subject into a lavish Elizabethan court portrait, preserving her facial likeness precisely. Dress her in an extravagant deep sapphire velvet court gown with elaborate antique-gold embroidery, pearl-strung bodice, cream lace cuffs and a magnificent sculptural lace ruff. Crown the silhouette with an ornate pearl-and-gold jewelled headdress, feathers and richly dressed period hair. Pose her facing us in a stately, reserved court portrait, head upright and both eyes clearly visible. Frame her closely while keeping the complete sculptural ruff and jeweled headdress visible. Her face remains large and central rather than being diminished by the costume. With the serene, self-possessed expression of a royal court portrait. Deep saturated sapphire, peacock blue, crimson accents, luminous pearl, ivory and antique gold dominate the image. Romantic painterly photography with luminous skin, rich directional candlelight, glowing golden highlights, deep velvety shadows and jewel-like color. An opulent Elizabethan great hall dissolves behind her into dark tapestries, carved wood, distant candlelight and warm amber bokeh. make the skin slightly pale Make the portrait sumptuous, theatrical and highly idealized, like an Elizabethan royal panel portrait brought vividly to life. Subtle aged photographic finish with softened blacks, warm highlights and gentle painterly grain while retaining saturated color and exquisite detail. Preserve the subject's face, age, ethnicity, proportions and natural asymmetry. age the photo with filters so it feels more like a painterly photo and older No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings. expression to be more in period, stately, reserved`,
    avoid: null,
  },
  ...
  persian_court: {
    id:    'persian_court',
    body:  `Transform the subject into a sumptuous romantic portrait of a Persian nobleman of the Safavid court, preserving his facial likeness and ethnicity. Dress him in richly woven crimson, madder red and tarnished-gold brocade, layered period garments, a jeweled sash, restrained antique jewelry and an elegant Safavid turban. Likeness is important.
Likeness is essential. maintain expression, micro gestures, imperfections. do not introduce body imperfections.
Do not modify body or head shape to suit period. DO NOT INCREASE WEIGHT
He belongs completely to the period, with a natural, reserved courtly posture and quiet, self-possessed expression. Avoid modern posing and contemporary styling.
Photograph him in an intimate Safavid palace chamber by dim oil-lamp and candlelight, with carved plaster, faded Persian textiles, dark wood and intricate tilework disappearing into shadow.
face should occupy at least 15% of the image. frame from thighs to top of head plus 20% margin above head and sides.
Give the photograph a softly aged, time-worn character: faded crimson, tarnished gold, tobacco brown, warm umber and muted lapis. Slightly desaturated skin and colors, warm lifted blacks, amber haze, subtle grain and softened highlights. Deep velvety shadows with uneven flame falloff.
...
Graceful three-quarter pose, with her face turned naturally toward us and both eyes clearly visible. Frame her closely from the upper chest to the top of the head, keeping her face large and dominant in the image. Serene expression. Luminous warm skin, soft romantic light, glowing highlights and deep velvety shadows. An opulent Safavid palace interior dissolves behind her into warm lamplight and golden bokeh. Rich crimson, antique gold and warm umber, softly aged and painterly. Luxurious, romantic and highly idealized, like a historic Persian oil portrait brought vividly to life. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
  ...
  renaissance_woman: {
    ...
    body:  `make the subject a realistic photo of a Renaissance noblewoman. warm earth palette of umber, ochre, deep red and black. velvet gown with a squared low neckline over a fine linen chemise, full sleeves tied at the shoulder with the chemise puffing through the lacing, hair parted at the centre and dressed with a fine pearl net, a single gold chain. no ruff. Woman facing forward, head upright and both eyes clearly visible, with the shoulders only gently relaxed rather than strongly turned. Zoom in for an upper-torso portrait. Her head should occupy roughly 30% of the image height. calm, settled expression. do not modify ethnicity. Background: a Renaissance interior — a plastered wall in warm ochre, a carved walnut chest, a heavy tapestry with faded figures, a leaded window off to the left throwing one shaft of light. deep shadow, warm dim air, slightly out of focus. Flattering soft key light, shadow separating jaw from neck. Clear the skin — blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.
desaturate skin colors 15% and add a filter to age the image slightly
The likeness is essential. Maintain micro facial gestures, imperfections, changes in symmetry. all the characteristics that make this person this person. The hair should be organically faithful in messy or organized condition.`,
    avoid: null,
  },
...
Photographic realism with the richness and visual poetry of a great painted portrait — atmospheric, tactile and subtly painterly without becoming an illustration. Rich restrained color, beautiful falloff, shallow depth of field, softly impressionistic Japanese setting. Idealized and striking while remaining completely believable as a photograph. Preserve facial likeness and permanent features. Frame from mid-chest to the top of the head, facing us directly with both eyes clearly visible, the armor and expression dominant against the softly blurred setting. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: null,
  },
  ...
  samurai_woman: {
    id:    'samurai_woman',
    body:  `Professional cinematic portrait of a female samurai in authentic Edo-period blue lacquered armor, layered lamellar plates and thick silk lacing, with the convincing weight, wear and imperfection of real historical armor. frame subject from thighs to top of helmet. Subject is drawing a period bow and has quiver on back with beautiful white fletched feathers. Hair tied beneath the helmet with a few loose strands escaping. use foreshortening and strong dynamic lighting. age the photo with filters Strong late-afternoon sunlight sweeps across her from the side, sculpting the face and armor with luminous highlights and deep expressive shadows. Photographic realism with the richness and visual poetry of a great painted portrait — atmospheric, tactile and subtly painterly without becoming an illustration. Rich restrained color, beautiful falloff, shallow depth of field, softly impressionistic background. Idealized and striking while remaining completely believable as a photograph. Preserve facial likeness and permanent features. Worn jewellery is fine: necklaces, earrings, piercings. subject has a strong focused expression. Use high constrast dramatic lighting. Show strain in the arms and face from drawing the bow.
desaturate the subjects skin tone by 15%. run a filter to antique the photo by 10%`,
    avoid: null,
  },
...
Preserve the subject's face, age, proportions and natural asymmetry. Refine temporary skin imperfections without changing his identity. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: null,
  },
...
Preserve the subject's face, age, proportions and natural asymmetry. Refine temporary skin imperfections without changing her identity. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
  ...
  wild_west_woman: {
    id:    'wild_west_woman',
    body:  `make the subject a realistic photo of a frontier woman of the American West, 1880s. muted palette of dust, tobacco brown, faded indigo and oxblood — low in contrast, no clean whites, everything worn and sun-faded. a high-collared calico bodice buttoned to the throat, a small cameo at the collar, a wool shawl over the shoulders. hair pinned up but loosening, strands falling around the face. Woman facing front, head upright and both eyes clearly visible, with only a gentle three-quarter turn of the shoulders. Frame from the upper chest to the top of the head, with her head occupying roughly 30% of the image height. calm, settled expression, weathered skin. do not modify ethnicity. Likeness is essential. Background: a saloon back room — plank walls, a stove, bottles on a shelf, one dirty window off to the left. deep shadow, warm dim light falling off fast. heavily out of focus, only shapes and glow readable. Flattering soft key light, shadow separating jaw from neck. Clear the skin — blemishes, spots and blotchiness go. Keep permanent structure: lines, scars and the natural asymmetry of the face. Add nothing that is not in the source. Never reshape, enlarge eyes, correct asymmetry or de-age. No held objects — no cups, glasses, phones, books, tools, watches or eyewear. Worn jewellery is fine: necklaces, earrings, piercings.`,
    avoid: null,
  },
