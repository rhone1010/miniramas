...
// authoritative 27 (never Ebony, Fire Face or Mercury) and shows them one
...
const universeSrc = (() => {
  const at = HTML.indexOf('\nvar CURATED_UNIVERSE = [')
  if (at < 0) throw new Error('CURATED_UNIVERSE not found')
...
function build(selected: string[] = []) {
  // every one of the 27 is a registry effect in some room
  const SILOS = [{ id: 'room', name: 'Room', effects: AUTHORITATIVE_27.map(id => ({ id, label: id })) }]
  const SELECTED = selected.map(b => ({ key: b, baseId: b }))
  const src = [
    universeSrc, 'var CURATED = { bench: [], page: 0 };',
...
describe('the Curated universe', () => {
  it('is exactly the authoritative 27, with no Ebony, Fire Face or Mercury', () => {
    const s = build()
    expect(s.CURATED_UNIVERSE.slice().sort()).toEqual(AUTHORITATIVE_27.slice().sort())
  ...
  it('every preview file the page can ask for is present', () => {
    for (const b of AUTHORITATIVE_27) for (const g of ['man', 'woman']) {
      expect(() => readFileSync(path.join(process.cwd(), 'public', 'previews', 'curated', b, g + '.jpg'))).not.toThrow()
    }
  })
})

describe('the bench', () => {
  it('draws 16 distinct effects from the 27, page 1 first', () => {
    for (let n = 0; n < 50; n++) {
      const s = build(); s.drawBench()
      expect(s.CURATED.bench).toHaveLength(16)
      expect(new Set(s.CURATED.bench).size).toBe(16)
      expect(s.CURATED.bench.every((id: string) => AUTHORITATIVE_27.includes(id))).toBe(true)
  ...
  it('a restored bench is accepted only if it is 16 distinct effects of the 27', () => {
    const s = build(); s.drawBench()
  ...
  for (const chosenCount of [0, 1, 3, 8, 15]) {
    it(`with ${chosenCount} chosen: the chosen stay put, every open place changes, never to what it held`, () => {
      for (let n = 0; n < 200; n++) {
        ...
        expect(after.every((id: string) => AUTHORITATIVE_27.includes(id))).toBe(true)
        for (let i = 0; i < 16; i++) {
