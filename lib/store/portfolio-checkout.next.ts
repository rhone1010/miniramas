...
export async function activatePortfolio(purchaseId: string): Promise<void> {
  const { error } = await supabaseAdmin.rpc('activate_discovery_portfolio', { p_purchase: purchaseId })
  if (error) throw new Error(`portfolio_activation_failed: ${error.message}`)
  ...
  console.log(`[activatePortfolio] purchase=${purchaseId} activated`)
}
