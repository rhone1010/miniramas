...
- Curator's actual copy/voice — logic is built (see below), but every
  user-facing string is a DRAFT placeholder pending Rich's approval.
  Not "not started" — built, but not launch-ready on copy.
...
## 8. CURATOR RECOMMENDATION LOGIC — BUILT THIS SESSION

`lib/store/curator.ts` no longer throws `curator_not_implemented`.
Real logic now: `analyzeSourcePhoto` (OpenAI vision, same pattern as
`classifySubject`) + `selectRecommendedEffects` (deterministic
selection from the 56-position catalog, silo-affinity scored from
photo analysis + intent text + quick-choice, per-silo caps, excludes
the known `made_by_hand` gap). Covers all three Curator entry paths
(Pick for me / Help me choose / Describe what you're looking for).

**"Pick for me" ASKS which size (1/4/8/16) — confirmed by Rich, does
NOT default automatically.** This was the open question from the 28
Aug carryover; it's resolved, not open anymore.

All message strings and the 6 quick-choice option labels are DRAFT —
real logic, placeholder copy, pending Rich's voice before ship.
