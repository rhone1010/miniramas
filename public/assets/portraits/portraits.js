...
/* Punch-list C (2026-07-11) — empty→suggested transition signal. The .main mural
   holds through upload + analyze; when the suggested cards are ready we flag the
   shell (data-suggested-ready) so integration.css fades the mural + deals the cards
   in. Observing #curEffectCards keeps it in lockstep with the engine's render. */
(function(){
  var grid  = document.getElementById('curEffectCards');
  var shell = document.getElementById('workshopShell');
  if(!grid || !shell) return;
  function sync(){ shell.setAttribute('data-suggested-ready', grid.children.length > 0 ? 'yes' : 'no'); }
